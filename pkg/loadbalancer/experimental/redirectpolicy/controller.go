...
func (pi *podInfo) hash() uint64 {
	...
	xxh.Reset()
	xxh.WriteString(pi.namespacedName)
...
func (h *lrpController) run(ctx context.Context, health cell.Health) error {
	...
	// To avoid processing irrelevant changes to pods, keep track of hashes
	// of the relevant info in a pod. We're only holding onto the hashes to
	// minimize state duplication and memory use.
	...
	for {
		...
		for change := range podChanges {
			...
			// Calculate a hash of the relevant data in the pod and see if it
			// has changed. This avoids the more expensive matching below for
			// irrelevant pod changes (like state updating).
			if !change.Deleted {
				podHash := podInfo.hash()
				oldHash, ok := podHashes[podInfo.namespacedName]
				if ok && podHash == oldHash {
					// No relevant changes in the pod.
					continue
				}
				podHashes[podInfo.namespacedName] = podHash
			} else {
				delete(podHashes, podInfo.namespacedName)
			}
...
func (pa *podAddr) writeHash(xxh *xxhash.Digest) {
	xxh.Write(pa.L3n4Addr.Bytes())
	xxh.WriteString(pa.portName)
}
