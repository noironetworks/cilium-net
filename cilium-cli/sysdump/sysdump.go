	...
	if c.Options.CiliumNamespace != "" {
		...
		if c.Options.CollectLogsFromNotReadyAgents {
			...
			for _, pod := range allPods {
				skip := false
				for _, p := range c.CiliumNotReadyPods {
					if pod.Name == p.Name {
						skip = true
					}
				}
...
func filterRunningNotReadyPods(l *corev1.PodList, limit int) []*corev1.Pod {
	return filterPods(l, func(po *corev1.Pod) bool {
		...
		ready := false
		for _, conditions := range po.Status.Conditions {
			if conditions.Type == corev1.PodReady && conditions.Status == corev1.ConditionTrue {
				ready = true
			}
		}
		return !ready
